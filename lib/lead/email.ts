...
function nameOrCompanyLine(lead: LeadSubmission): string {
  const company = lead.company.trim();
  if (company) return `${lead.name} / ${company}`;
  return lead.name;
}

export function getLeadEmailSubject(lead: LeadSubmission): string {
  return `🔴 NEUE ANFRAGE: von ${nameOrCompanyLine(lead)}`;
}
...
/** HTML-E-Mail für LEAD_EMAIL_RECIPIENT – tabellenbasiertes „Grid“. */
export function buildGfLeadNotificationHtml(lead: LeadSubmission): string {
  const company = lead.company.trim() || "—";
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:separate;border-spacing:0;background:#09090b;border-radius:10px;overflow:hidden;">
      <tr>
        <td width="50%" style="padding:14px 16px;border-right:1px solid rgba(248,250,252,0.15);">
          ...
          <div style="margin-top:6px;font-size:16px;font-weight:600;">${phoneCell}</div>
        </td>
  ...
  <td colspan="2" style="padding:6px;">
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:separate;border-spacing:0;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;">
      <tr><td style="padding:10px 14px 4px;font-size:11px;font-weight:600;letter-spacing:0.06em;text-transform:uppercase;color:#64748b;">Anliegen</td></tr>
      <tr><td style="padding:0 14px 14px;font-size:14px;line-height:1.5;color:#0f172a;white-space:pre-wrap;">${escapeHtml(lead.message)}</td></tr>
    </table>
  </td>
</tr>`;
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;background:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 12px 48px rgba(15,39,69,0.12);">
          <tr>
            <td style="background:linear-gradient(135deg,#f97316 0%,#09090b 100%);padding:22px 26px;">
              <p style="margin:0;font-size:12px;font-weight:600;color:#fed7aa;letter-spacing:0.12em;text-transform:uppercase;">Saubermatik · Anfrage</p>
